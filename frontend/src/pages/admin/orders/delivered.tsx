  ...
  const handleComplete = async (orderId: number) => {
    try {
      const response = await orderAPI.updateOrderStatus(orderId.toString(), { 
        status: 'DaHoanThanh',
        adminNote: 'Đơn hàng đã được hoàn thành' 
      });
      
      if (response.status === 'success') {
        showNotification('success', 'Đơn hàng đã được hoàn thành');
              ...
              {loading ? (
                ...
                filteredOrders.map((order) => (
                  <tr key={order.MaDonHang} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${order.TrangThaiThanhToan === 'DaThanhToan' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                        {order.TrangThaiThanhToan === 'DaThanhToan' ? 'Đã thanh toán' : 'Chưa thanh toán'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          onClick={() => handleComplete(order.MaDonHang)}
